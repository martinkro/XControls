...
void XLogger::setLogFileName(const QString& name)
{
    ...
    this->logFilePath = QDir::toNativeSeparators(logDirPath + QDir::separator() + logFileName);
...
void XLogger::messageHandler(QtMsgType type, const QMessageLogContext& context, const QString& message)
{
    ...
    if (getInstance().getLogFilePath().isEmpty())
    {
        getInstance().setLogFileName("log.log");
    }
